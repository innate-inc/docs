...
4. **Enter the SSH connection string** using the credentials and IP from the [Connecting via SSH](/main/robots/mars/connecting-via-ssh) page

5. **Open the workspace folder** once connected (e.g., `/home/jetson1/innate-ws/`)

For default credentials, IP addresses, and connection methods (WiFi vs USB-C), see [Connecting via SSH](/main/robots/mars/connecting-via-ssh).
...
For quick edits or debugging, you can SSH directly into MARS using terminal-based editors like `nano` or `vim`. See [Connecting via SSH](/main/robots/mars/connecting-via-ssh) for connection details.
...
[PreviousOverview](/main/software/overview)[NextSkills](/main/software/skills)
