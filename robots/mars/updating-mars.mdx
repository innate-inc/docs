...
This page explains how to update the software running on your MARS robot using the `innate update` command.
...
A new robot OS version may require you to update the [Innate Controller App](/robots/innate-controller-app) for full compatibility.

If your update includes stereo-depth changes, run [Stereo Depth Calibration](/robots/mars/stereo-depth-calibration) before normal navigation.

***

## Prerequisites

* You can **SSH into your robot**. If WiFi isn't available, you can also [connect via the Jetson USB-C port](/robots/mars/connecting-via-ssh#via-usb-c-jetson).
