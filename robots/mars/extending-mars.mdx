...
Users can integrate additional sensors on the available ports. For any added device, make sure it is mounted and installed properly on the robot. You can then feed its data into the Innate agent by writing an [input device](/software/inputs) — a small Python class dropped into `~/innate-os/workspace/inputs/`.
...
MARS can already listen with its built-in microphone. If you want more directional pickup, plug in a directional microphone and follow the [Inputs](/software/inputs#example-the-built-in-microphone) guide.
