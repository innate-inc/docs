---
title: "One-Time Fix for Early Units (MARS IDs 1-11)"
---

<Info>
This page documents a one-time fix for early MARS units with [MARS IDs](/robots/mars/faq#how-do-i-find-my-mars-id) **1-11** (shipped before January 16, 2026). If your MARS ID is 12 or higher — or you've already applied this fix — it does not apply to you, and this page is kept for reference only.
</Info>

Early MARS units (IDs 1-11) require a one-time update before normal operation. This update must be applied via a Wi-Fi hotspot connection before using the standard app-based setup.
...
3. Tap on it and just stay on the next screen.
...
* **Robot doesn't connect to hotspot:** Make sure the hotspot name is exactly `FBI_VAN_6` (matching upper/lower case) and the password is exactly `12345678`
