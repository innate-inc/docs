...
<Note>This section is for advanced users familiar with ROS2. If you're not comfortable with ROS, simply reach out to us on [Discord](https://discord.com/invite/KtkyT97kc7) and we'll help you diagnose the issue.</Note>
...
Some fixes and shipped-hardware questions depend on which unit you have. Two ways to find its ID:

* **Via Bluetooth** — turn the robot on and look for it in the Innate app's Bluetooth connections screen. It appears as **"MARS the 1st"**, **"MARS the 5th"**, **"MARS the 12th"** — the number in the name is your MARS ID.
* **Via SSH** — read the `robot_id` field from the robot info file:
...
<Note>
The first eleven units (MARS IDs 1–11, shipped before January 2026) needed a one-time setup fix before first use. Every robot from ID 12 onwards shipped with it already applied. If you have an early unit that was never set up, reach out on [Discord](https://discord.com/invite/KtkyT97kc7).
</Note>
