...
<Steps>
  <Step title="Update to the latest stable OS">
    ...
    OS `0.6.0` and later include the hackathon tooling (previously distributed as the `0.5.0-rc12` dev release — no longer needed).
  </Step>
</Steps>
...
- **Latest Android APK** — available at [cdn.innate.bot/innate-app-latest-1.3.0.apk](https://cdn.innate.bot/innate-app-latest-1.3.0.apk). The iOS app is available on [TestFlight](https://testflight.apple.com/join/YeChe4A7). Leader Arm operation only works on Android. We have some Android phones to share if no one on your team has one.
...
<Card
  title="MARS URDF (mars_sim)"
  icon="cube"
  href="https://github.com/innate-inc/innate-os/tree/main/ros2_ws/src/mars_bot/mars_sim"
>
