...
<Note>
A measured version of this loop — scored rollouts with live inference charts, failure tags, and separate evaluation datasets — is taking shape on the web app's **Profiling** page, along with an opt-in `auto_stop` flag in a skill's `metadata.json` that ends a run when the policy's progress head says it's done. Both live on `innate-os` `main` and are **not in a released OS build yet**.
</Note>
