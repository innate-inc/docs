...
Trained skills are available to agents and code-defined skills just like any other skill. Once a skill is deployed, the robot generates a typed reference for it under `physical_skills`, so you can name it the same way you name a code skill:

```python
from innate import Agent, SkillRef
from innate_skills.navigate_to_position import NavigateToPosition
from physical_skills import PickUpCup          # generated for this robot
...
Full ID strings (`"local/pick_up_cup"`) still work, and are what you want when the skill isn't on this robot yet.

A code-defined skill can run the policy directly, with the same call shape as any [sub-skill](/software/skills/code-defined-skills/composing-skills):

```python
class TidyUp(Skill):
    """Pick up a cup and put it away."""
...
Either way the execution pipeline handles loading the checkpoint, running inference, and sending commands to the hardware.
