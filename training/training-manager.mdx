...
<Info>
  The Training Manager is **experimental**: it works and ships with the OS, but
  expect rough edges. For the standard record-train-deploy workflow, the
  [app's training UI](/training/overview) is the polished path. Contributions welcome.
</Info>
