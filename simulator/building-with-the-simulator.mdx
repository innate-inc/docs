...
connected to a physics world instead of motors and cameras.

That container doesn't have its own copy of the software. It runs directly
from the `innate-os` folder you cloned during [setup](/simulator/setup) —
your checkout is mounted into it live, so editing files on your machine
changes the running robot, no image rebuilds involved.

For building [skills](/software/skills) and [agents](/software/agents), the
part of the checkout that matters is `workspace/`: drop Python files into
`workspace/custom_skills/` or `workspace/custom_agents/` and the robot
hot-reloads them the moment you save. (Deeper changes, like the ROS nodes
under `ros2_ws/`, need a build step — the table further down covers what
takes effect when.) And because a real robot runs the same `workspace/`,
whatever you build here deploys to hardware unchanged.
