...
<Steps>
  <Step title="Create the skill file">
    With the simulator running, create
    `workspace/custom_skills/victory_spin.py` in your innate-os checkout:
...
Skills become interesting when the agent can decide to use them. Drop a
minimal agent next door in `workspace/custom_agents/cheerful.py`:
