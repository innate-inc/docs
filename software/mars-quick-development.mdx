...
Go to `~/innate-os/workspace/custom_agents/` and create a `hello_world.py` agent file:
...
Save the file, then restart the runtime with `innate service restart` — new files are picked up on restart, and later edits to them hot-reload without restarting. Open the app, and your agent appears on the Home screen: start it, sit in front of the robot, and observe!
...
Below is an example of how to create a skill that queries gmail to read the last emails. Create it as `~/innate-os/workspace/custom_skills/retrieve_emails.py`:
