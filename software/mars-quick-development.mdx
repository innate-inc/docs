...
As an example, let's give MARS a skill that reads your latest Gmail messages. Grab the complete `RetrieveEmails` implementation from the [Digital Skills worked example](/software/skills/code-defined-skills/digital-skills#worked-example-retrieveemails) and save it as `~/innate-os/workspace/custom_skills/retrieve_emails.py` on the robot — it's copy-paste-ready, you only need to fill in your Gmail address and an app password.
