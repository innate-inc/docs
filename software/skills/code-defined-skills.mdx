...
`.data` is anything you like — a dict, or a small `pydantic` model when you want the caller's editor to know the fields. The shipped `turn_in_place` skill does this, which is why a caller can write `result.data.turned_degrees`.
