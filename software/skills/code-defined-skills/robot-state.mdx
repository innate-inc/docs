...
A plain annotation is **required** — the server waits for the first value and fails the run up front if none arrives, so no `None` guards are needed inside `execute()`. Appending `| None` makes the feed **best effort** instead: injected when available, `None` otherwise, and every read must be guarded.
...
`self.odom` drifts and resets every boot but is always available; for the persistent map frame (the coordinates `navigate_to_position` targets), declare `pose: Pose` — it reads `None` until the robot is localized, so declare it `Pose | None` unless localization is guaranteed.
...
self.arm.position            # end-effector (x, y, z); .gripper is the claw joint
...
A skill that captures images while rotating. The first example spells out `guidelines()` explicitly; the camera and mobility are declared with bare annotations, so both are guaranteed inside `execute()`:
...
class MonitorPosition(Skill):
    """Use to monitor robot position for a duration."""
    ...
    def execute(self, duration: float = 5.0) -> SkillReturn:
        ...
        while time.time() - start_time < duration:
            ...
            self.feedback(f"Moved {distance:.2f}m from start")
            time.sleep(0.5)
