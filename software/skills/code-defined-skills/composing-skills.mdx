...
[code-defined skill](/software/skills/code-defined-skills): the class docstring is its
`guidelines()`, it declares [robot state](/software/skills/code-defined-skills/robot-state)
with a type annotation (`battery: Battery | None`), persists data across runs with
`self.storage`, speaks with `self.say(..., wait=True)`, and returns a plain success
message.
