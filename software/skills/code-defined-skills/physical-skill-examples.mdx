...
Rotate to find a person, then wave. The first example spells out `guidelines()` explicitly and shows the cancellation helpers:

```python
import math
from innate import MainImage, Manipulation, Mobility, Head, Skill, SkillResult, SkillReturn
...
class ScanAndWave(Skill):
    ...
    def execute(self) -> SkillReturn:
        # Stop the base the instant a cancel lands
