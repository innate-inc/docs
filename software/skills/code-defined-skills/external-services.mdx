...
Many service operations are atomic and cannot be meaningfully cancelled. If your skill has nothing to interrupt, just don't check for cancellation — a cancel that lands mid-run takes effect at the next `self.check_cancelled()` call, so a skill without checkpoints simply runs to completion. Mention it in the guidelines if the agent should know:
