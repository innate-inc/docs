...
Every motion method blocks until the arm settles and raises `ArmFailed` (pose unreachable, command rejected) or `ArmUnhealthy` (servo fault) on failure — no return codes to check. Import the exceptions from `innate` when you want to handle them:
...
The `strength` you close with becomes the **standing grip target**: every following `move_to` / `follow` / 5-joint `move_joints` carries it automatically, so a gripped object stays gripped with no extra bookkeeping. Grip once, then move freely.
