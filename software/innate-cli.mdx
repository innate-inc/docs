...
Start the Foxglove bridge to visualize topics, TF, images, and point clouds from a browser. Off by default on the robot; always on in the [simulator](/simulator/building-with-the-simulator#foxglove-and-ros-tooling). See [Foxglove Setup](/software/foxglove-setup) for the full workflow.

| Command | What it does |
| --- | --- |
| `innate foxglove start` | Start the bridge (WebSocket on port `8765`); prints the `ws://` URL to connect to |
| `innate foxglove stop` | Stop the bridge |
| `innate foxglove status` | Show whether the bridge is running |
...
innate foxglove start           → start Foxglove bridge (ws :8765)
innate foxglove stop            → stop the bridge
innate foxglove status          → bridge status
