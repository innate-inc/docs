        ...
        def loop():
            while not self._stop.is_set():
                ...
                for event_text in events:
                    # Async feedback to the Innate agent during agent execution.
                    # send_data expects a dict; "chat_in" payloads carry a "text" key.
                    self.send_data({"text": event_text}, data_type="chat_in")
                self._stop.wait(1.0)
...
When this agent starts, `on_open()` is called and the Innate agent begins receiving these asynchronous updates.
