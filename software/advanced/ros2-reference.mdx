...
Velocity commands (linear.x, angular.z) to drive the robot

`/odom`

`nav_msgs/Odometry`

Robot odometry (position, orientation, velocity)
...
Main RGB camera (front-facing OAK-D)
...
Alternative topic for main camera
...
Direct arm joint position commands

`/leader/state`

`sensor_msgs/JointState`

Leader arm joint positions (when connected)

`/joint_states`

`sensor_msgs/JointState`

Combined joint states for visualization
...
Incoming WebSocket messages from cloud agent

`/ws_outgoing`

`std_msgs/String`

Outgoing WebSocket messages to cloud agent
...
Move arm to joint positions over a specified duration (seconds)

`/mars/head/set_ai_position`

`std_srvs/Trigger`

Move head to default AI viewing position

**GotoJS Request:**

```text
std_msgs/Float64MultiArray data  # Target joint positions
float64 time                      # Duration in SECONDS (e.g. 2.0 = 2 s, not 2000)
---
bool success
```
...
`maurice_msgs/LightCommand`

Control robot LED lights

`/shutdown`

`maurice_msgs/Shutdown`

Trigger system shutdown

`/set_robot_name`

`maurice_msgs/SetRobotName`

Set robot display name

`/trigger_update`

`maurice_msgs/TriggerUpdate`
...
`maurice_msgs/GridLocalize`

Estimate robot pose on map using laser scan

`/change_map`

`brain_messages/ChangeMap`

Switch to a different saved map

`/save_map`

`brain_messages/SaveMap`

Save the current map

`/delete_map`

`brain_messages/DeleteMap`

Delete a saved map

`/change_navigation_mode`

`brain_messages/ChangeNavigationMode`
...
`brain_messages/GetAvailablePrimitives`

List all available skills

`/get_available_directives`

`brain_messages/GetAvailableDirectives`

List all available agents

`/get_available_behaviors`

`brain_messages/GetAvailableBehaviors`

List manipulation behaviors

`/reset_brain`

`brain_messages/ResetBrain`

Reset the brain system

`/set_directive_on_startup`

`brain_messages/SetDirectiveOnStartup`

Set which agent loads on boot

`/get_chat_history`

`brain_messages/GetChatHistory`
...
`brain_messages/ManipulationTask`

Start recording a new manipulation task

`/new_episode`

`std_srvs/Trigger`

Start a new episode in current task
...
Finish recording task

`/get_task_metadata`

`brain_messages/GetTaskMetadata`

Get task info

`/get_dataset_info`

`brain_messages/GetDatasetInfo`
...
`brain_messages/ExecuteBehavior`

Execute a learned manipulation behavior

`/execute_policy`

`brain_messages/ExecutePolicy`

Execute a policy for a set duration
...
request.time = 2.0  # duration in SECONDS (2.0 = 2 s)
...
[https://integrations.gitbook.com/v1/integrations/mermaid/integration?v=153](https://integrations.gitbook.com/v1/integrations/mermaid/integration?v=153)

***

## Further Reading

* [Troubleshooting via tmux](/robots/mars/troubleshooting) — debug running nodes

* [Connecting via SSH](/robots/mars/connecting-via-ssh) — access the robot terminal
