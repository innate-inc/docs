...
<Steps>
  ...
  <Step title="SSH in">
    ...
    # e.g. for MARS the 21st:
    ssh jetson1@mars-the-21st.local
    ```
    Password: `goodbot<number>` — `goodbot21` for MARS the 21st, `goodbot27` for MARS the 27th. See [Connecting via SSH](/robots/mars/connecting-via-ssh) for hostname, Ethernet, and USB-C options.
  </Step>
...
# e.g. ssh-copy-id jetson1@mars-the-27th.local
```
</Tip>
