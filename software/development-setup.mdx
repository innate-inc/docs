...
MARS auto-launches all ROS2 nodes on boot via systemd. Edits to existing agent, skill, and input files in `workspace/` hot-reload while the robot is running. **You need to restart when you add new files or change anything outside `workspace/`** (ROS2 nodes, drivers, launch files):
