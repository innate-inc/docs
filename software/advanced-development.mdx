...
<Steps>
  ...
  <Step title="Rebuild">
    Recompile and restart in one command:

    ```bash
    innate rebuild
    ```

    Or rebuild just the package you changed:

    ```bash
    innate rebuild maurice_arm
    ```
  </Step>
  <Step title="Verify">
...
If a node crashes, its pane shows the error. Fix the code, rebuild the package, and restart.
...
Everything in `~/innate-os/ros2_ws/src/` is a ROS2 package. After editing, rebuild:

```bash
innate rebuild <package_name>
```
...
- **Build only what changed.** `innate rebuild maurice_arm` is faster than `innate rebuild`.
