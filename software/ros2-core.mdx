...
* **brain\_client**: The main orchestrator — runs [the Innate agent](/software/agent) loop on the robot, loads agents and skills, and executes them
...
<Steps>
  ...
  <Step title="Start Foxglove Bridge">
    On the robot, run `innate foxglove start` — it exposes topics over WebSocket and prints the `ws://<robot-ip>:8765` URL to connect to.
  </Step>
  <Step title="Open Foxglove and connect">
    In your browser, open [app.foxglove.dev](https://app.foxglove.dev) and connect to the printed `ws://` URL.
  </Step>
</Steps>

See the full [Foxglove Setup](/software/foxglove-setup) guide for topics to visualize and the Wi-Fi bandwidth caveat.
