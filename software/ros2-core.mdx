...
**"I want to read raw camera data"** → Subscribe to `/mars/main_camera/left/image_raw` (sensor\_msgs/Image)
...
<Note>
  When visualizing remotely, subscribe to the throttled topics under `/mars/main_camera/remote/…` instead of the full-rate `/mars/main_camera/…` topics. The remote variants are rate-limited to avoid saturating the network link. See [Foxglove Setup](/software/foxglove-setup) for details.
</Note>
