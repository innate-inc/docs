...
Legacy locations (`~/skills/`, `~/innate-os/skills/`) from releases before the workspace layout are still scanned, and you can add extra directories via the `[paths]` section of `config/os.toml`. Skills from any of these also get the `local/` prefix — only `innate_skills/` produces `innate-os/` IDs.
