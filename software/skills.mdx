...
When the Innate agent receives a request like "check on grandma," it decomposes this into a skill chain: navigate to bedroom → look around → wave → speak reassurance. One request, one coherent behavior.
...
| Getting around | |
| --- | --- |
| `navigate_to_position` | Drive to x, y coordinates — on the map, or relative to where the robot stands with `local_frame=True`. |
