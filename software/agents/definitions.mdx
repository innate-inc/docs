...
`get_skills()` takes full skill IDs, prefixed by where each skill comes from:
...
IDs are matched exactly during registration — a custom skill named
`victory_spin` must be listed as `"local/victory_spin"`, not bare.
