...
Check that the file is in `~/innate-os/workspace/custom_agents/` and that you restarted the runtime after adding it (`innate service restart`), then on the app **Home** screen swipe down (pull-to-refresh) to reload the agent list. If it still does not appear, verify the file imports without Python errors and inspect runtime logs using the [Troubleshooting guide](/robots/mars/troubleshooting#navigate-tmux-to-find-issues).
